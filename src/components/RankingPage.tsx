      ...
      {loading ? (
        ...
        <main className="mx-auto max-w-7xl px-4 py-6 sm:py-8 sm:px-6 lg:px-8">
          ...
          {activeTab === 'tiobe' ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-primary-50 to-blue-50 dark:from-primary-900/30 dark:to-blue-900/30 border-b border-gray-200 dark:border-gray-700">
                      <th className="px-3 sm:px-6 py-3 sm:py-4 text-center text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider" style={{ width: '80px' }}>
                        {language === 'zh' ? '排名' : 'Rank'}
                      </th>
                      <th className="px-3 sm:px-6 py-3 sm:py-4 text-center text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider" style={{ width: 'auto' }}>
                        {language === 'zh' ? '编程语言' : 'Language'}
                      </th>
                      <th className="px-3 sm:px-6 py-3 sm:py-4 text-center text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider" style={{ width: '140px' }}>
                        {language === 'zh' ? '流行度' : 'Rating'}
                      </th>
                    </tr>
                  </thead>
                      ...
                      >
                        <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                          <div className="flex justify-center">
                            <div
                              className={`inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-xl font-bold text-sm sm:text-base ${getRankBadgeColor(item.rank)}`}
                            >
                              {item.rank <= 3 ? <Trophy className="h-4 w-4 sm:h-5 sm:w-5" /> : item.rank}
                            </div>
                          </div>
                        </td>
                        <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                          <div className="flex justify-center items-center">
                            <div className="text-base sm:text-lg font-bold text-gray-900 dark:text-gray-100">{item.language}</div>
                          </div>
                        </td>
                        <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                          <div className="flex justify-center">
                            <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-1.5 sm:py-2 rounded-full bg-gradient-to-r from-primary-100 to-blue-100 dark:from-primary-900/40 dark:to-blue-900/40 border border-primary-200 dark:border-primary-700">
                              <TrendingUp className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-primary-600 dark:text-primary-400" />
                              <span className="text-sm sm:text-base font-bold text-primary-700 dark:text-primary-300">
            ...
            /* GitHub Trending - Mobile Card View & Desktop Table */
            <div>
              {/* Mobile Card View */}
              <div className="md:hidden space-y-3 sm:space-y-4 animate-fade-in-up">
                {trendingRepos.map((repo, index) => (
                  <div
                    key={repo.rank}
                    className="bg-white dark:bg-gray-800 rounded-xl shadow-card border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-card-hover hover:border-primary-300 dark:hover:border-primary-600 transition-all duration-300 p-4 animate-fade-in-up"
                    style={{ animationDelay: `${index * 30}ms` }}
                  >
                    {/* Rank Badge */}
                    <div className="flex items-start justify-between mb-3">
                      <div
                        className={`inline-flex items-center justify-center w-10 h-10 rounded-xl font-bold text-sm ${getRankBadgeColor(repo.rank)}`}
                      >
                        {repo.rank <= 3 ? <Trophy className="h-4 w-4" /> : repo.rank}
                      </div>
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-blue-100 to-indigo-100 dark:from-blue-900/40 dark:to-indigo-900/40 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-700">
                        {repo.language}
                      </span>
                    </div>
                    ...
                    >
                      <h3 className="text-base font-bold text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 hover:underline line-clamp-1">
                        {repo.name}
                      </h3>
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-gradient-to-r from-yellow-100 to-orange-100 dark:from-yellow-900/40 dark:to-orange-900/40 border border-yellow-200 dark:border-yellow-700">
              ...
              {/* Desktop Table View */}
              <div className="hidden md:block bg-white dark:bg-gray-800 rounded-xl shadow-card border border-gray-200 dark:border-gray-700 overflow-hidden animate-fade-in-up max-w-6xl mx-auto">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="bg-gradient-to-r from-primary-50 to-blue-50 dark:from-primary-900/30 dark:to-blue-900/30 border-b border-gray-200 dark:border-gray-700">
          ...
          {/* Footer Note */}
          <div className={`mt-6 p-3 sm:p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 mx-auto ${activeTab === 'tiobe' ? 'max-w-4xl' : 'max-w-6xl'}`}>
