...
export function RankingPage() {
  ...
  const getRankBadgeColor = (rank: number) => {
    ...
    return 'bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 text-gray-700 dark:text-gray-300';
  };
                    ...
                    {rankings.map((item, index) => (
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div
                            className={`inline-flex items-center justify-center w-10 h-10 rounded-lg font-bold text-sm ${getRankBadgeColor(item.rank)}`}
                          >
                            {item.rank <= 3 ? <Trophy className="h-4 w-4" /> : item.rank}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">{getRankChange(item.change)}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div className="font-semibold text-gray-900 dark:text-gray-100">{item.language}</div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-400 font-semibold text-sm">
                            <TrendingUp className="h-3 w-3" />
                            {item.rating.toFixed(2)}%
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-600 dark:text-gray-400 font-medium">
                          #{item.prevRank}
                        </td>
                      </tr>
                    ))}
